...
(function (global) {
  ...
  function brief(R) {
    ...
    if (global.Persona) {
      ...
      L.push('남에게 보이는 이미지: ' + P.image.join(' '));
  ...
  var SOLO_SPEC = {
    ...
    mind: '겉으로 보이는 모습과 속에 있는 모습이 어떻게 다른지. 위에 준 "남에게 보이는 이미지"를 근거로, 남이 보는 나와 실제 나의 간극을 한 문단 짚어라. 건강운에서 나온 약한 곳이 성격과 어떻게 맞물리는지도 한 줄 넣어라. 화가 나는 지점, 무너지는 지점, 혼자 있을 때의 상태, 스트레스를 처리하는 방식. 오행 편중과 십신 구성이 만드는 사고 습관과 감정 패턴을 구체적으로 짚어라. 이 사람이 자기 자신에 대해 착각하고 있을 법한 것도 하나 지적한다. 1000자 이상.',
