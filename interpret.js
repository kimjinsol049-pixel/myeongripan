...
(function (global) {
  ...
  function brief(R) {
    ...
    if (global.Persona) {
      ...
      L.push('');
      L.push('[화면에 이미 보여 준 값 — 어긋나게 말하지 말 것]');
      L.push('올해(' + P.thisYear.year + ') 세운 ' + P.thisYear.gz + ' · ' + P.thisYear.note.join(' / '));
      L.push('올해 연애가 움직이는 달: ' + (P.thisYear.months.length
        ? P.thisYear.months.map(function (m) { return m.m + '(' + m.gz + ', ' + m.why.join('·') + ')'; }).join(', ')
        : '두드러지는 달 없음'));
      L.push('연애 스타일(일지 ' + P.loveStyle.god + '): ' + P.loveStyle.main +
        (P.loveStyle.extra.length ? ' / ' + P.loveStyle.extra.join(' / ') : ''));
      L.push('만나는 사람: ' + P.partner.lines.join(' '));
      L.push('건강운 ' + P.health.score + '점(' + P.health.grade + ') — ' + P.health.notes.join(' / ') +
        (P.health.care.length ? ' / 조심할 곳: ' + P.health.care.map(function (c) { return c.organ; }).join(', ') : ''));
      L.push('첫인상(일간 ' + S.STEM_H[R.dm] + ' — ' + P.firstLook.key + '): ' + P.firstLook.lines.join(' ') +
        (P.firstLook.gap ? ' / 실제로는: ' + P.firstLook.gap : ''));
      L.push('외모 특징: ' + P.looks.items.map(function (it) { return it.part + ' — ' + it.d; }).join(' ') +
        (P.looks.marks.length ? ' / ' + P.looks.marks.join(' ') : ''));
      L.push('남에게 보이는 이미지(' + P.image.role + '): ' + P.image.lines.join(' ') +
        (P.image.heard.length ? ' / 자주 듣는 말: ' + P.image.heard.join(', ') : '') +
        (P.image.misread.length ? ' / 오해받는 지점: ' + P.image.misread.join(' ') : ''));
      L.push('어울리는 옷: ' + P.fashion.color + ' / ' + P.fashion.item + ' / 피할 색 ' + P.fashion.avoidColor);
      L.push('닮은 동물: ' + P.animal.name + ' — ' + P.animal.desc);
    }
    return L.join('\n');
  }
  ...
  var SOLO_SPEC = {
    ...
    mind: '겉으로 보이는 모습과 속에 있는 모습이 어떻게 다른지. 위에 준 "첫인상"과 "남에게 보이는 이미지"를 근거로, 처음 3초에 읽히는 모습 → 몇 달 뒤 굳는 평판 → 실제 속 순서로 세 겹을 벌려 놓고 그 간극을 한 문단 짚어라. "자주 듣는 말"과 "오해받는 지점"을 실제 상황에 넣어 보여라. 건강운에서 나온 약한 곳이 성격과 어떻게 맞물리는지도 한 줄 넣어라. 화가 나는 지점, 무너지는 지점, 혼자 있을 때의 상태, 스트레스를 처리하는 방식. 오행 편중과 십신 구성이 만드는 사고 습관과 감정 패턴을 구체적으로 짚어라. 이 사람이 자기 자신에 대해 착각하고 있을 법한 것도 하나 지적한다. 1000자 이상.',
    ...
    love: '연애할 때의 실제 행동, 끌리는 상대의 유형, 관계가 깨지는 지점. 위에 준 올해 연애 시기(움직이는 달)와 만나는 사람 특징을 반드시 문단 안에 녹여라. 남자는 재성, 여자는 관성이 배우자성이니 그 상태(유무·강약·합충·공망)로 배우자운을 판정한다. 일지(배우자 자리)의 글자와 그 충형 여부를 반드시 해석한다. 결혼 시기, 결혼 후 달라지는 점, 이 사람이 관계에서 반드시 고쳐야 할 것 하나를 못 박는다. 1100자 이상.',
  ...
  var PAIR_SPEC = {
    ...
    p_love: '연애와 결혼 관점. 두 사람의 연애 스타일이 어떻게 부딪히고 맞물리는지, 올해 각자의 연애 시기가 겹치는지 엇갈리는지도 짚어라. 일지(배우자궁) 관계를 중심으로 애정의 온도, 성적 궁합의 결, 생활 리듬의 합을 판정한다. 남자는 재성, 여자는 관성 상태를 상대와 대조한다. 결혼까지 갈 관계인지, 간다면 무엇이 조건인지, 안 간다면 어디서 끊어지는지 못 박는다. 1000자 이상.',
  ...
  var GROUP_SPEC = {
    ...
    g_love: '이 사람들 중 연인으로 묶었을 때 가장 잘 맞는 조합을 순위로 매긴다. 위에 준 시기 예측(첫 연애·결혼 나이)이 있으면 누가 먼저 움직이고 누가 기다리는지도 엮어서 말한다. 성별이나 현재 관계와 무관하게 순수하게 사주 상성만으로 판정한다. 1위 조합이 왜 1위인지 일간·일지·용신 보완으로 설명하고, 반대로 절대 연인으로 엮이면 안 되는 조합도 하나 못 박는다. 각 조합이 실제로 사귀면 어떤 모습일지 장면으로 쓴다. 900자 이상.',
  ...
  function groupBrief(list, G) {
    ...
    if (global.Fortune) {
      var F = global.Fortune.rank(list);
      L.push('');
      L.push('[재미로 뽑은 시기 예측 — 화면에 이미 순위로 보여 준 값이다]');
      F.profiles.forEach(function (p) {
        L.push('- ' + p.name + ': 첫 연애 ' + p.firstLove.age + '세(' + p.firstLove.year + '년), ' +
          '결혼 ' + p.marriage.age + '세(' + p.marriage.year + '년, 결혼 의지 ' + p.marriage.will + '/100), ' +
          '연애 횟수 약 ' + p.count.n + '번, 평균 연애 기간 ' + p.duration.label + ', ' +
          '취직 ' + p.job.age + '세(' + p.job.path + ')');
      });
      L.push('※ 이 수치는 대운과 원국 구조로 뽑은 어림값이다. 해석에서 언급할 때는 단정하되 "사주가 가리키는 시기"라는 틀을 유지하고, 화면 숫자와 어긋나게 말하지 마라.');
    }
    return L.join('\n');
  }
