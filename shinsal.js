...
(function (global) {
  ...
  var DESC = {
    ...
    // ── 중립 ──
    도화: '이성에게 눈에 띄는 별. 매력과 인기가 따르지만 구설과 삼각관계도 같이 온다. 연예·영업·서비스에서는 무기다.',
    ...
    // ── 흉살 ──
    양인: '칼날의 별. 추진력과 결단이 최고지만 그 칼에 자기가 베인다. 극단으로 치닫기 쉽다.',
    비인: '양인의 반대편. 숨은 칼. 갑작스러운 사고와 수술, 급한 성질.',
    괴강: '우두머리의 기질. 극단적으로 강하고 결벽하다. 크게 되거나 크게 꺾인다. 여자는 고집이 세다고 본다.',
    백호: '피를 보는 별. 사고·수술·혈광지사. 조상 중에 비명횡사가 있다고도 본다. 대신 기가 세서 큰일을 한다.',
    ...
    원진: '까닭 없이 미운 관계. 이유를 대기 어려운 짜증과 원망이 쌓인다.',
  ...
  function detect(P, gongmang) {
    ...
    branches.forEach(function (b, i) {
      ...
      if (YANGIN[dm] === b) push(out, '양인', '흉', tag);
      if (YANGIN[dm] !== undefined && (YANGIN[dm] + 6) % 12 === b) push(out, '비인', '흉', tag);
    ...
    [yearB, dayB].forEach(function (base) {
      ...
      branches.forEach(function (b, i) {
        ...
        if (name === '년살') { push(out, '도화', '중', TAGS[i] + '지'); return; }
    ...
    for (var i = 0; i < branches.length; i++) for (var j = i + 1; j < branches.length; j++) {
      if (hasPair(PAIR.원진, branches[i], branches[j])) push(out, '원진', '흉', TAGS[i] + '지·' + TAGS[j] + '지');
    ...
    P.forEach(function (p, i) {
      var tag = TAGS[i] + '주';
      if (GWAEGANG.some(function (g) { return g[0] === p.s && g[1] === p.b; }) && i === 2) push(out, '괴강', '흉', tag);
      if (BAEKHO.some(function (g) { return g[0] === p.s && g[1] === p.b; })) push(out, '백호', '흉', tag);
